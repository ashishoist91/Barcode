...
class product_barcode_print(report_sxw.rml_parse):
    ...
    def _generateBarecodeString(self, default_code, ean13):  #, height, width):
        barcode_string = ''
        if default_code:
            barcode_string += default_code
        if ean13:
            barcode_string += ean13
    ...
    def _generateBarcode(self, barcode_string):  #, height, width):
        ...
        Code128().getImage(barcode_string, path="/opt/odoo").save(fp, "PNG")
        barcode_data = base64.b64encode(fp.getvalue())
        return barcode_data
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
           ...
           # 'getLabelRows':self._getLabelRows,
            ...
            'generateBarecodeString': self._generateBarecodeString,
        })


class report_purchase_barcode_print(osv.AbstractModel):
    _name = 'report.product_barcode_print.report_product_barcode_template'
    _inherit = 'report.abstract_report'
    _template = 'product_barcode_print.report_product_barcode_template'
